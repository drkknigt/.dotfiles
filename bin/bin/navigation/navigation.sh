...
if [ "$dotfile_flag" = "0" ] ; then 
    ...
    file_to_open=$(fdfind  ~ --full-path --type f --hidden  | fzf --prompt="search from home directory: " --bind "ctrl-f:preview-down,ctrl-u:preview-up" --bind "ctrl-o:toggle-preview" --preview="tree -L 1 {} | batcat --theme='Monokai Extended Origin' --color=always" --preview-window hidden)
fi
