...
fzf_lua.setup({
    ...
    files = {
        ...
        -- cmd = "fdfind --absolute --type f -H --exclude .git --exclude .config --exclude .wine",
        -- find_opts         = [[-type f -not -path '*/\.git/*' -printf '%P\n']],
        rg_opts = "--color=never --files --hidden --follow -g '!.git'",
        fdfind_opts = "--color=never --type f --hidden --absolute-path --exclude .git .config .mozilla .wine",
        -- find_opts         = [[-type f -not -path '*/\.git/*' -printf '%P\n']],
        -- rg_opts           = "--color=never --files --hidden --follow -g '!.git'",
...
-- vim.ui.select opts
fzf_lua.register_ui_select(function(ui_opts, _)
    return {
        prompt = "❯ ",
        winopts = {
            height = 0.2,
